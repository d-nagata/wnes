...
os.makedirs(save_dir_path+"/outlier_fig/png", exist_ok=True)
os.makedirs(save_dir_path+"/outlier_fig/eps", exist_ok=True)
os.makedirs(save_dir_path+"/outlier_fig/png_log", exist_ok=True)
os.makedirs(save_dir_path+"/outlier_fig/eps_log", exist_ok=True)
...
for i in range(seeds):
    ...
    start=0
    end=100#len(fitness_mins)
    
    plt.rcParams.update({'font.size': 18})
    plt.figure(figsize=(10, 6))
    plt.plot(range(start, end), fitness_mins[start:end], label='min', color='dodgerblue')#green
    plt.plot(range(start, end), fitness_maxs[start:end], label='max', color='green')#green
    plt.plot(range(start, end), fitness_ratios[start:end], label='ratio', color='red')#green
    ...
    plt.savefig(save_dir_path+'/outlier_fig/png/'+func+'_seed'+str(i) +'.png')
    plt.savefig(save_dir_path+'/outlier_fig/eps/'+func+'_seed'+str(i) +'.eps')
    plt.yscale('log')
    plt.savefig(save_dir_path+'/outlier_fig/png_log/'+func+'_seed'+str(i) +'_log.png')
    plt.savefig(save_dir_path+'/outlier_fig/eps_log/'+func+'_seed'+str(i) +'_log.eps')
