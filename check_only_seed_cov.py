...
for i in range(seeds):
    if i==1:
        ...
        ax2 = ax1.twinx()
        ax2.plot(range(start, end), conv_eig_mins[start:end], label='eigs_min', color='red')
        ax2.plot(range(start, end), conv_eig_maxs[start:end], label='eigs_max', color='green')
        ax2.plot(range(start, end), conv_eig_ratios[start:end], label='eigs_ratio', color='magenta')
        ax2.set_yscale('log')
        ax2.set_ylabel('eigs')
        plt.legend(loc='lower left')

        save_message = "check_div_fitness"
