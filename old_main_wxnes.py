...
def main(cfg:DictConfig):
    ...
    save_path = "./wxnes_data/"+func+"/"+current_date
    ...
    for dim in dims:
        ...
        for seed in range(seed_num):
            optimizer = WXNES(mean=mean+np.zeros(dim),population_size=pop_size,eta=eta, sigma=sigma, seed=seed)
            ...
            for generation in range(max_steps):
                ...
                mean_for_save,mean_raw_grad,updated_count = optimizer.tell(generation, solutions)
                # print(f"mean: {mean}")
                # print(f"C: {C}")
